...
def run(simulation_folder):
    ...
    population = []
    # TODO: expand_steps = 2  # Expand the initial population for some steps
    ...
    keep_top_k = 4
    mutate_count = population_size//2

    # Run simulation loop
    evaluation_process = random_lm_eval.RandomEval(dataset_dir=simulation_folder+"/datasets",
                                                   sample_size=20)
    selection_process = roulette_wheel_based_selection.RankBasedSelection(keep_top_k=keep_top_k)
    ...
    for iteration in range(target_iterations):
        ...
        selected_pairs, new_population = selection_process.run_selection(population)
